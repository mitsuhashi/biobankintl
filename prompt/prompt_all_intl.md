...
* ステップ１及び２ともUKBBの6.1から7.1まで及びステップ3: 残存条文の処理を実施してください。
...
*   **対応する条文が存在する場合:**
    ...
    *   **統合条文**
        ...
        *  色分けと日本語訳の手順: 統合条文（「外国統合英」および「外国統合日」）を作成する際は、以下の手順を厳密に守ってください。
          ...
          *  次に、その英語条文に対して、以下の色分けルールを適用し、HTMLの<span style='color: ...'>タグで文言を囲みます。これが最優先です。
               * UKBB由来の文言: color: black;
               * EGA由来の文言: color: blue;
               * NIH由来の文言: color: green;
               * 統一用語：文脈上、明らかに特定のMTAの条文をベースに、その中の単語を統一用語に置き換えただけの場合は、汎用化・再構成部分（赤）とせず、ベースとなったMTAの色を適用してください。
               * 汎用化・再構成部分（red）には以下を含むこと。
                 ...
                 *  構造の再編: 複数の条文から要素を抜き出して新しい一文を構成した場合の、文全体の骨格部分。
                 *  逆に、由来MTAの文章から単語やフレーズをほぼそのまま流用している場合は、たとえ文中の一部であっても由来MTAの色（black, blue, green）を適用すること。
                 *  単数形・複数形の変更、冠詞の追加・削除、動詞の時制変更など、軽微な文法的調整は、由来元の色を維持してください。
               * 最後に、色分けされた英語の各<span>ブロックに正確に対応する日本語訳を作成してください。日本語訳の色分けは、英語の<span>構造に可能な限り忠実に従うものとしますが、言語的な違いから完全な一対一対応が著しく不自然な翻訳を生む場合は、意味的なまとまりを優先して翻訳し、対応する色を適用してください。
...
* 原文の色は以下の通りにしてください。
  ...
  * EGA原文: color: blue;
  * NIH原文: color: green;
*   原文を表示する条文中で、特定の国・地域に固有の法律、規制、機関名（例: Human Tissue Act 2004, NIH, EU embargo）に関する文書は太字にしてください。統一用語は<strong>タグで囲まずに太字にしないでください。複数の条文が１つのセルに入る場合は、先頭に条文番号を書き、次の文との間に改行を入れてください。
